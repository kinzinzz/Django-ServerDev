...
class RegisteUser(APIView):
    def post(self, request):
        serializers = LoginUserSerializer(request.data)
        
        if LoginUser.objects.filter(user_id=serializers.data['user_id']).exists():
            # DB에 있는 값 출력할 때 어떻게 나오는지 보려고 user 객체에 담음
            user = LoginUser.objects.filter(user_id=serializers.data['user_id']).first()
            data = dict(
                msg="이미 존재하는 아이디입니다.",
        ...
        user = serializers.create(request.data)
